...
public class Overview {
	...
	public void changeCenterToMain(GridPane grid){
	        ...
	        if(pb_cap_val <= 0.5) { pb_cap.getStyleClass().add("red-bar"); } else { pb_cap.getStyleClass().add("green-bar"); };
	        if(pb_zfh_doz_val <= 0.5) { pb_doz.getStyleClass().add("red-bar"); } else { pb_doz.getStyleClass().add("green-bar"); };
	        if(pb_zfh_stud_val <= 0.5) { pb_stud.getStyleClass().add("red-bar"); } else { pb_stud.getStyleClass().add("green-bar"); };
	        if(pb_quali_val <= 0.5) { pb_quali.getStyleClass().add("red-bar"); } else { pb_quali.getStyleClass().add("green-bar"); };
			...
			Label l_partner = new Label("Partnerunternehmen\n" + this.partner/*, Grafik.png*/);
